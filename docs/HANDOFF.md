...
| Next | More real content, and distribution. A ce-ideate run concluded the remaining engineering is hygiene and the real bottleneck is content and audience. |
...
| Convex | Dev `strong-turtle-110`, prod `exciting-deer-586`, team `lecturesfrom`, project `automationsanonymous`. Functions deployed to both. `ADMIN_SESSION_SECRET` set on both. Both hold the six authored automations and twelve tools; no placeholders remain in either. |
...
- Public writes are gated twice, on purpose. `src/lib/payload-check.ts` runs in the request path and reaches the form, the API and the MCP tool through the one shared schema; `convex/lib/limits.ts` repeats the cheap half as a backstop, because `NEXT_PUBLIC_CONVEX_URL` is public and the mutation is callable without touching Next.js. The gate checks that a payload is what it claims to be, which a bot generating plausible prose fails and a person who actually ran the automation passes without noticing. Turnstile remains an unwired hook point and is now largely redundant: it could only ever have defended the browser path.
- `publish` refuses a record whose tool slugs have no row, because they render as links and would ship live 404s. Unknown slugs are deliberately allowed at submit time; the operator adds the tool during review.
- Three of the site's own automations are now records (`screenshot-every-route-after-every-change`, `check-your-api-still-matches-your-website`, `fail-the-build-when-published-content-stops-being-valid`). Their payloads are read from the actual scripts, so CI checks the same bytes the corpus publishes.
...
- The test scripts create records titled PLACEHOLDER on purpose. `scripts/clear-placeholders.sh [--prod]` removes them; run it after `scripts/e2e-flows.sh` or an agent-surface write test. Both deployments are currently clean.

### Open threads

1. Create a Convex production deploy key in the dashboard and add it to GitHub as the `CONVEX_DEPLOY_KEY` repository secret. The `deploy-convex` job in `.github/workflows/ci.yml` is already written and skips with a message until the secret exists; adding it is the whole remaining step, and it closes the standing gap where `main` could carry Convex functions production did not have.
